...
export function findRubricLevel(percentage) {
    const roundedPercentage = Math.floor(percentage);
    const matchedLevel = rubric.find((item) => roundedPercentage >= item.minPercentage && roundedPercentage <= item.maxPercentage);
    if (!matchedLevel) {
        throw new Error(`No rubric level found for ${percentage}%.`);
    }
    return matchedLevel;
}
export function calculateAcademicResult(subjects) {
    ...
    const aggregateLevel = subjectResults.reduce((sum, subject) => sum + subject.rubricLevel.academicLevel, 0);
    return {
        totalMaximum,
        totalObtained,
        aggregateLevel,
